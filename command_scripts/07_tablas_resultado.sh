...
# Generamos el tsv con los campos decididos, en este caso : "CHROM","POS","REF","ALT","QUAL","DP","GQ","CONSEQUENCE","IMPACT","GENE","MANE","CLIN_SIG","gnomAD_AF"
# En primer lugar se extraen los campos del VCF.
bcftools query -f '%CHROM\t%POS\t%REF\t%ALT\t%QUAL\t[%DP]\t[%GQ]\t%INFO/CSQ\n' /data/processed/annotated/Archivo_anotado.vcf | 
...
awk -F'\t' 'BEGIN{
OFS="\t";
print "CHROM","POS","REF","ALT","QUAL","DP","GQ","CONSEQUENCE","IMPACT","GENE","MANE","CLIN_SIG","gnomAD_AF"
}
{
split($8, csq, "|");
print $1,$2,$3,$4,$5,$6,$7,csq[2],csq[3],csq[4],csq[27],csq[71],csq[58]
...
# Se puede comprobar con grep que la columna de las frecuencias sea la numero 13
awk -F'\t' 'NR==1 || $13=="" || $13 < 0.01' /results/tables_var/Tabla_variantes.tsv > /results/tables_mut/Tabla_mutaciones.tsv